    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Object useDefaultServer;
    private boolean _useDefaultServerType;
    private Object secure;
    private boolean _secureType;
    private Object accessKey;
    private String _accessKeyType;
    private Object secretKey;
    private String _secretKeyType;
    private static Logger logger = LoggerFactory.getLogger(CreateObjectUriMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createObjectUri"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (accessKey!= null) {
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, accessKey));
            } else {
                if (castedModuleObject.getAccessKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createObjectUri"), event, new RuntimeException("You must provide a accessKey at the config or the message processor level."));
                }
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, castedModuleObject.getAccessKey()));
            }
            if (secretKey!= null) {
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, secretKey));
            } else {
                if (castedModuleObject.getSecretKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createObjectUri"), event, new RuntimeException("You must provide a secretKey at the config or the message processor level."));
                }
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, castedModuleObject.getSecretKey()));
            }
            String transformedBucketName = ((String) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            String transformedKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_keyType").getGenericType(), null, key));
            Boolean transformedUseDefaultServer = ((Boolean) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_useDefaultServerType").getGenericType(), null, useDefaultServer));
            Boolean transformedSecure = ((Boolean) evaluateAndTransform(_muleMessage, CreateObjectUriMessageProcessor.class.getDeclaredField("_secureType").getGenericType(), null, secure));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createObjectUri"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.createObjectUri(transformedBucketName, transformedKey, transformedUseDefaultServer, transformedSecure);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("createObjectUri"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
