    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Object versioningStatus;
    private org.mule.module.s3.simpleapi.VersioningStatus _versioningStatusType;
    private Object accessKey;
    private String _accessKeyType;
    private Object secretKey;
    private String _secretKeyType;
    private static Logger logger = LoggerFactory.getLogger(SetBucketVersioningStatusMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("setBucketVersioningStatus"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (accessKey!= null) {
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, SetBucketVersioningStatusMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, accessKey));
            } else {
                if (castedModuleObject.getAccessKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("setBucketVersioningStatus"), event, new RuntimeException("You must provide a accessKey at the config or the message processor level."));
                }
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, SetBucketVersioningStatusMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, castedModuleObject.getAccessKey()));
            }
            if (secretKey!= null) {
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, SetBucketVersioningStatusMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, secretKey));
            } else {
                if (castedModuleObject.getSecretKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("setBucketVersioningStatus"), event, new RuntimeException("You must provide a secretKey at the config or the message processor level."));
                }
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, SetBucketVersioningStatusMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, castedModuleObject.getSecretKey()));
            }
            String transformedBucketName = ((String) evaluateAndTransform(_muleMessage, SetBucketVersioningStatusMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            org.mule.module.s3.simpleapi.VersioningStatus transformedVersioningStatus = ((org.mule.module.s3.simpleapi.VersioningStatus) evaluateAndTransform(_muleMessage, SetBucketVersioningStatusMessageProcessor.class.getDeclaredField("_versioningStatusType").getGenericType(), null, versioningStatus));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("setBucketVersioningStatus"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            connection.setBucketVersioningStatus(transformedBucketName, transformedVersioningStatus);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("setBucketVersioningStatus"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
