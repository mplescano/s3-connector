...
/**
 * GetBucketPolicyMessageProcessor invokes the {@link org.mule.module.s3.S3Connector#getBucketPolicy(java.lang.String)} method in {@link S3Connector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetBucketPolicyMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetBucketPolicyMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getBucketPolicy"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (accessKey!= null) {
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, GetBucketPolicyMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, accessKey));
            } else {
                if (castedModuleObject.getAccessKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("getBucketPolicy"), event, new RuntimeException("You must provide a accessKey at the config or the message processor level."));
                }
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, GetBucketPolicyMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, castedModuleObject.getAccessKey()));
            }
            if (secretKey!= null) {
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, GetBucketPolicyMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, secretKey));
            } else {
                if (castedModuleObject.getSecretKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("getBucketPolicy"), event, new RuntimeException("You must provide a secretKey at the config or the message processor level."));
                }
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, GetBucketPolicyMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, castedModuleObject.getSecretKey()));
            }
            String transformedBucketName = ((String) evaluateAndTransform(_muleMessage, GetBucketPolicyMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getBucketPolicy"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getBucketPolicy(transformedBucketName);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getBucketPolicy"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
