    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private org.mule.module.s3.simpleapi.Region _regionType;
    private Object acl;
    ...
    private static Logger logger = LoggerFactory.getLogger(CreateBucketMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createBucket"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (accessKey!= null) {
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, accessKey));
            } else {
                if (castedModuleObject.getAccessKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createBucket"), event, new RuntimeException("You must provide a accessKey at the config or the message processor level."));
                }
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, castedModuleObject.getAccessKey()));
            }
            if (secretKey!= null) {
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, secretKey));
            } else {
                if (castedModuleObject.getSecretKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createBucket"), event, new RuntimeException("You must provide a secretKey at the config or the message processor level."));
                }
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, castedModuleObject.getSecretKey()));
            }
            String transformedBucketName = ((String) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            org.mule.module.s3.simpleapi.Region transformedRegion = ((org.mule.module.s3.simpleapi.Region) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_regionType").getGenericType(), null, region));
            org.mule.module.s3.AccessControlList transformedAcl = ((org.mule.module.s3.AccessControlList) evaluateAndTransform(_muleMessage, CreateBucketMessageProcessor.class.getDeclaredField("_aclType").getGenericType(), null, acl));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createBucket"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.createBucket(transformedBucketName, transformedRegion, transformedAcl);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("createBucket"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
