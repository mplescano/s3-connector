    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(CreateObjectPresignedUriMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createObjectPresignedUri"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (accessKey!= null) {
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, accessKey));
            } else {
                if (castedModuleObject.getAccessKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createObjectPresignedUri"), event, new RuntimeException("You must provide a accessKey at the config or the message processor level."));
                }
                transformedAccessKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_accessKeyType").getGenericType(), null, castedModuleObject.getAccessKey()));
            }
            if (secretKey!= null) {
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, secretKey));
            } else {
                if (castedModuleObject.getSecretKey() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createObjectPresignedUri"), event, new RuntimeException("You must provide a secretKey at the config or the message processor level."));
                }
                transformedSecretKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_secretKeyType").getGenericType(), null, castedModuleObject.getSecretKey()));
            }
            String transformedBucketName = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            String transformedKey = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_keyType").getGenericType(), null, key));
            String transformedVersionId = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_versionIdType").getGenericType(), null, versionId));
            Date transformedExpiration = ((Date) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_expirationType").getGenericType(), null, expiration));
            String transformedMethod = ((String) evaluateAndTransform(_muleMessage, CreateObjectPresignedUriMessageProcessor.class.getDeclaredField("_methodType").getGenericType(), null, method));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createObjectPresignedUri"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.createObjectPresignedUri(transformedBucketName, transformedKey, transformedVersionId, transformedExpiration, transformedMethod);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("createObjectPresignedUri"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
